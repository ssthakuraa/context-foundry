...
- [-] Implement strict schemas and generated/static types for source captures, locators, assertions, coverage, releases, task artifacts, human decisions, errors and evaluation manifests. Sixteen schemas, static types and portable JSON exports exist; strict first-party payload checks cover `engineering.symbol`, `engineering.relationship` and `business.rule`, plus Gate 1 `scope_map` and post-Gate 1 `sufficiency` bodies. Task artifact body digests and immediate prior versions are verified. Metadata-only binding covers distinct sources, missing files, revision drift, locator/file digest mismatch and canonical declared-file manifest digest closure. Source locators have evidence IDs; `checkSupportClosure` rejects missing/duplicate support, undeclared relationship support, cycles and oversized closures, and returns bounded transitive evidence IDs only for valid graphs. `checkReleaseIntegrity` withholds that set if capture binding fails. Evidence: 34 tests, typecheck, build, 16-schema check and 46 HTML page check passed on 2026-09-20. Broader record kinds, findings/proposal/completion bodies, non-source evidence, actual-byte verification and authorization remain; schema validity alone never grants authority.
- [x] Implement initial TypeScript canonical serialization and SHA-256 digests; verify Unicode, number, duplicate decoded keys, record ordering, round trips and unsupported variants. Evidence: `packages/contracts/src/canonical.ts` and portable [canonical vectors](packages/contracts/fixtures/canonical-vectors.json) with fixed canonical UTF-8/digest expectations; all 33 package tests passed on 2026-09-20. Independent non-TypeScript conformance and contract freeze remain open.
