...
- [-] Implement strict schemas and generated/static types for source captures, locators, assertions, coverage, releases, task artifacts, human decisions, errors and evaluation manifests. Twelve schemas, static types and portable JSON exports exist; captured-file, release-set and relationship-payload shapes were added. Metadata-only binding checks cover distinct sources, missing files, revision drift, locator/file digest mismatch and canonical declared-file manifest digest closure. Source locators now have evidence IDs; `checkSupportClosure` rejects missing/duplicate support, undeclared relationship support, cycles and oversized closures, and returns a bounded transitive evidence-ID set only for valid graphs. `checkReleaseIntegrity` withholds that set if capture binding fails. The 31-test receipt passed on 2026-09-20. Broader kind-specific payloads, non-source evidence, actual-byte verification and authorization remain; schema validity alone never grants authority.
