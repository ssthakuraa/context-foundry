...
- [-] Implement strict schemas and generated/static types for source captures, locators, assertions, coverage, releases, task artifacts, human decisions, errors and evaluation manifests. Fourteen schemas, static types and portable JSON exports exist; strict first-party payload checks now cover `engineering.symbol`, `engineering.relationship` and `business.rule`. Metadata-only binding covers distinct sources, missing files, revision drift, locator/file digest mismatch and canonical declared-file manifest digest closure. Source locators have evidence IDs; `checkSupportClosure` rejects missing/duplicate support, undeclared relationship support, cycles and oversized closures, and returns bounded transitive evidence IDs only for valid graphs. `checkReleaseIntegrity` withholds that set if capture binding fails. Broader record kinds, task bodies, non-source evidence, actual-byte verification and authorization remain; schema validity alone never grants authority.
