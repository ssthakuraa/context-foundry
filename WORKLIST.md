...
- [-] Implement strict schemas and generated/static types for source captures, locators, assertions, coverage, releases, task artifacts, human decisions, errors and evaluation manifests. Nineteen schemas, static types and portable JSON exports exist; strict first-party payload checks cover `engineering.symbol`, `engineering.relationship` and `business.rule`, plus all five task artifact bodies. Task artifact body digests, prior versions and exact same-task findings/proposal references are checked by `checkTaskArtifactReferences`; this does not verify approval. Metadata-only binding covers distinct sources, missing files, revision drift, locator/file digest mismatch and canonical declared-file manifest digest closure. Source locators have evidence IDs; `checkSupportClosure` rejects missing/duplicate support, undeclared relationship support, cycles and oversized closures, and returns bounded transitive evidence IDs only for valid graphs. `checkReleaseIntegrity` withholds that set if capture binding fails. Evidence: 35 tests, typecheck, build, 19-schema check and 46 HTML page check passed on 2026-09-20. Broader record kinds, non-source evidence, actual-byte verification and authorization remain; schema validity alone never grants authority.
