...
- The current fifty-two package tests, typecheck, build, schema export check, forty-nine generated HTML pages, seventeen bounded semantic and three shape-invalid vectors in TypeScript/Python, nine valid/six rejected canonical JSON and three valid/three rejected JSONL vectors in TypeScript/Python pass. Generated 16 MiB size-boundary cases also pass in both languages. These are bounded fixtures, not a completeness claim.
...
| Portable semantic parity | JSON Schema exports do not encode all TypeScript semantic checks, including path normalization, cross-record support, exact body digest and version-chain rules | [Partial ledger](semantic-conformance.md) and seventeen semantic/three shape-invalid shared cases now cover six bounded rule families with independent Draft 7 prevalidation in Python; remaining single/cross-object parity is still required |
