...
- The current fifty-seven contract tests plus two isolated extractor-spike tests, typecheck, build, schema export check, fifty generated HTML pages, seventeen bounded semantic, three shape-invalid and six scope-map artifact variants in TypeScript/Python, nine valid/six rejected canonical JSON and three valid/three rejected JSONL vectors in TypeScript/Python pass. Generated 16 MiB size-boundary cases also pass in both languages. These are bounded fixtures, not a completeness claim.
