...
- The current sixty-four contract tests plus fourteen isolated extractor-spike tests, typecheck, build, schema export check, fifty generated HTML pages, thirty bounded semantic, three shape-invalid and six scope-map artifact variants in TypeScript/Python, nine valid/six rejected canonical JSON and three valid/three rejected JSONL vectors in TypeScript/Python pass. Generated 16 MiB size-boundary cases also pass in both languages. The TypeScript spike now assembles a deterministic multi-file candidate from caller-supplied bytes or a Linux explicit-root local read, with records/locators and partial file-unit coverage, and offers a local lexical symbol/pointer baseline; it cannot prove Git source origin, manifest completeness, publication authority or WP2/POV acceptance.

## Blocking gaps before a shared contract freeze

| Gap | Why it matters | Exit evidence |
|---|---|---|
| Kind catalog | The nine [proposed first-slice kinds](record-kind-catalog.md) have strict payloads and declared producer/consumer kind compatibility; interface implementation and typed engineering test targets now resolve against unique local symbols. Manifest authenticity, remaining target/reference types, declaration adapters and actual unsupported coverage remain | Each first-slice producer/consumer kind is strictly handled or explicitly unsupported with coverage and reference closure |
| Portable semantic parity | JSON Schema exports do not encode all TypeScript semantic checks, including path normalization, cross-record support, exact body digest and version-chain rules | [Partial ledger](semantic-conformance.md) and thirty semantic/three shape-invalid plus six scope-map artifact variants cover bounded single-object rules with independent Draft 7 prevalidation in Python; other body kinds and cross-object parity remain |
