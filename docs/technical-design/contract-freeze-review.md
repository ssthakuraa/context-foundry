...
- The current sixty-four contract tests plus eight isolated extractor-spike tests, typecheck, build, schema export check, fifty generated HTML pages, thirty bounded semantic, three shape-invalid and six scope-map artifact variants in TypeScript/Python, nine valid/six rejected canonical JSON and three valid/three rejected JSONL vectors in TypeScript/Python pass. Generated 16 MiB size-boundary cases also pass in both languages. The TypeScript spike now assembles a deterministic multi-file candidate from caller-supplied bytes with records/locators and partial file-unit coverage, but cannot prove source origin, manifest completeness or WP2 acceptance.
