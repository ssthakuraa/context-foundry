...
- The current forty-nine package tests, typecheck, build, schema export check, forty-nine generated HTML pages, seventeen bounded semantic vectors in TypeScript/Python and four valid/three rejected canonical vectors in TypeScript/Python pass. These are bounded fixtures, not a completeness claim.
...
| Evidence trust | Execution-evidence JSON is shape-only; a new pure raw-byte/digest comparator checks only bytes supplied by its caller, not source origin, capture completeness, producer authenticity or publication authority | Authenticated importer and digest/revision/policy receipts; forged or stale report denial tests |
