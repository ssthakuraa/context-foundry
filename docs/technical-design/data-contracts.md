...
`checkReleaseSetBindings` validates the release-set and supplied manifest shapes, then
requires one exact pack/release identity and canonical manifest digest per entry with
no ambiguous or unlisted manifest. This is metadata closure only: a claimed shard or
bridge digest is not evidence that bytes were loaded or reviewed. Byte verification,
cross-pack entity conflict detection and activation authorization remain separate.
`checkReleaseSetByteClosure` now compares all ordered, caller-supplied shard bytes
and the caller-supplied bridge bytes to those digests after metadata closure. Missing,
extra, duplicated or changed inputs fail the check. This is still not producer
authentication, source-manifest provenance, a compiled-generation conflict check,
review, or permission to activate.
