...
The stage audit now also runs a 32-seed/32 KiB typed counterfactual with the
same question, candidate and hop cap. It calls a miss `seed_selection` only when
that probe actually recovers the fact; if the probe cannot serialize, the audit
marks the classification incomplete. A new competing-pending-rule test exercises
this distinction. More seeds expose both passages but do not adjudicate which
rule applies. The counterfactual is offline evaluation, not an unrestricted
production retrieval request.
