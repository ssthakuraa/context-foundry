...
`checkBusinessFlowLinks` rejects missing, duplicate, unlisted and mismatched flow/step identities; `checkReleaseIntegrity` withholds its support closure if those links fail. Step precondition/outcome references, test targets and interface implementation pointers are not yet resolved by that check. Flow membership is a structural assertion, not proof of business sequence or cross-product behavior.
