...
export type StageLoss = {
  ...
  typed_with_wide_seed_cap: boolean;
...
export function compareRetrieval(candidate: CrossLayerCandidate, scenario: string,
  ...
  const typedFullWire = retrieveCandidate(candidate, { ...request, mode: 'typed',
    max_bytes: 32 * 1024 });
  // A second probe relaxes only seed and wire caps. A fact is attributed to
  // seed selection only when this probe actually recovers it.
  const typedWideSeeds = retrieveCandidate(candidate, { ...request, mode: 'typed',
    max_seeds: 32, max_bytes: 32 * 1024 });
  ...
  const typedWideSeedIds = new Set(typedWideSeeds.ok ?
    typedWideSeeds.packet.facts.map(item => item.record_id) : []);
  const obligations = expectedRecordIds.map(record_id => {
    ...
    const typed_with_wide_seed_cap = typedWideSeedIds.has(record_id);
    const loss_stage: StageLoss['loss_stage'] = !source_present ? 'preparation' :
      ...
      lexical_candidate && !selected_seed && !typedWideSeeds.ok ? 'evaluation_incomplete' :
      lexical_candidate && !selected_seed && typed_with_wide_seed_cap ? 'seed_selection' :
      'typed_traversal_or_budget';
    return { record_id, source_present, lexical_candidate, selected_seed,
      typed_with_full_wire_cap, typed_with_wide_seed_cap, typed_packet, loss_stage };
  });
  return { scenario, candidate_digest: candidate.digest,
    ...
    typed_wide_seed_bytes: typedWideSeeds.ok ? typedWideSeeds.bytes : null,
    ...
    ...(!typedWideSeeds.ok ? { typed_wide_seed_error: typedWideSeeds.code } : {}),
    obligations };
}
